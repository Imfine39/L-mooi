...
**Phase 1（今回のスコープ）:**
- [x] クライアント（支援先会社）管理（CRUD）
- [x] 補助金・申請書類・セクションの管理（CRUD）
- [x] セクション内容の入力・編集（AI生成はPhase 2）
- [x] 参考ファイルのアップロード・管理（既存事業/補助事業で区分）
- [x] 参考ファイルの手動紐づけ（選択・新規アップロード両対応）
...
| 2 | ダッシュボード | 全クライアント概要 | クライアント一覧、最近の編集、進捗サマリー |
| 3 | クライアント詳細画面 | クライアントの補助金一覧 | 補助金リスト、検索、新規作成 |
...
| 9 | 参考ファイル管理画面 | ファイルのアップロード・管理 | ファイルリスト（既存事業/補助事業区分）、アップロード、プレビュー |
| 10 | テンプレート管理画面 | 補助金テンプレートの編集 | 書類・セクション構造の編集 |
| 11 | ユーザー管理画面 | ユーザーの管理 | ユーザーリスト、招待 |
| 12 | クライアント管理画面 | クライアントの管理 | クライアントリスト、追加・編集 |

### 5.2 Screen Transitions (Provisional)

- ログイン画面 → ダッシュボード: ログイン成功
- ダッシュボード → クライアント詳細画面: クライアント選択
- ダッシュボード → クライアント管理画面: メニュー選択
- クライアント詳細画面 → 補助金詳細画面: 補助金選択
- 補助金詳細画面 → 書類詳細画面: 書類選択
